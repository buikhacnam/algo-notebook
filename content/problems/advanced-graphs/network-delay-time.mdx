...
We will send a signal from a given node <code>k</code>. Return _the minimum time it takes for all the_ <code>n</code> _nodes to receive the signal_. If it is impossible for all the <code>n</code> nodes to receive the signal, return <code>-1</code>.
