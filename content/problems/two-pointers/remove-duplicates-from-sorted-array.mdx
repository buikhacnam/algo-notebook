...
Given an integer array <code>nums</code> sorted in non-decreasing order, remove the duplicates <a href="https://en.wikipedia.org/wiki/In-place_algorithm" target="_blank">in-place</a> such that each unique element appears only once. The relative order of the elements should be kept the same. Then return _the number of unique elements in _<code>nums</code>.
