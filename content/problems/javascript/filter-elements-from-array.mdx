...
<code>filteredArr</code> should only contain the elements from the <code>arr</code> for which the expression <code>fn(arr[i], i)</code> evaluates to a truthy value. A truthy value is a value where <code>Boolean(value)</code> returns <code>true</code>.
