...
Return _an edge that can be removed so that the resulting graph is a tree of _<code>n</code>_ nodes_. If there are multiple answers, return the answer that occurs last in the input.
