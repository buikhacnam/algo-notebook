...
Given a binary tree <code>root</code>, a node _X_ in the tree is named good if in the path from root to _X_ there are no nodes with a value _greater than_ X.
