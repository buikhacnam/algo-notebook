...
Given an array of positive integers <code>nums</code> and a positive integer <code>target</code>, return _the minimal length of a _<span data-keyword="subarray-nonempty">_subarray_</span>_ whose sum is greater than or equal to_ <code>target</code>. If there is no such subarray, return <code>0</code> instead.
